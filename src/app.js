  ...
  if (reduced || !('IntersectionObserver' in window)) {
    ...
    var io = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        ...
        /* Если блок обнаружился уже в глубине экрана, значит читатель
           пролистнул рывком и подъем ему только мешает: показываем сразу.
           При обычной прокрутке эта ветка не срабатывает, блок попадает
           в наблюдение задолго до нижней кромки. */
    ...
    /* Порог нулевой: у высокого блока проценты его собственной высоты
       ...
       Сто шестьдесят пикселей ниже кромки — не на глаз, а по замеру.
       Отбивка между разделами сама по себе оставляет до 270 пикселей
       пустого экрана, и это предел верстки. При зоне выше кромки
       анимация добавляла к нему еще полтораста: заголовок был уже на
       экране, но держал прозрачность около нуля. На ста шестидесяти
       добавка обращается в ноль, дальше запас ничего не улучшает и
       только съедает подъем, который читатель мог бы застать. */
